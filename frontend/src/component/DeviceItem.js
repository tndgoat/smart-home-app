    ...
    return (
        <View style={styles.shadow} className='flex my-1 bg-[#D8E4E8] w-[280px] h-fit space-y-[5px] justify-center py-2 px-4 rounded-[16px] items-center '>
            <View className='flex w-full flex-row items-center justify-between '>
                <TouchableOpacity onPress={() => props.type === 'light' ? navigation.navigate('LightSetting', { name: props.name, location: props.location, color: props.color, status: props.status, schedule: props.schedule }) :
                    props.type === 'tv' ? navigation.navigate('SmartTVSetting', { name: props.name, location: props.location, channel: props.channel, state: props.state }) : null
                }>
            ...
            <View className='flex w-full flex-row justify-between '>
                ...
                {props.type === 'light' ?
                    <View className='flex flex-row'>
                        ...
                        <View className='flex flex-row justify-center items-center' >
                            <Text className="text-center font-regular text-[12px]">color number </Text>
                    ...
                    props.type === 'tv' ?
                        <View className='flex flex-row'>
                            <Image className='mr-4' source={require('../../assets/TVItem.png')}></Image>
                            <View className='flex flex-row justify-center items-center' >
                                <Text className="text-center font-regular text-[12px]">channel </Text>
                                <Text className="text-center font-semibold text-[16px]">{props.channel}</Text>
                            </View>
                        </View>
                        : <View></View>}
                ...
                <View className='flex space-y-[6px]'>
                    <TouchableOpacity onPress={() => setModalVisible(!modalVisible)} className='bg-white p-[3px] rounded-[4px]' style={styles.shadow} >
                    ...
                    <Modal
                        ...
                        <View className='flex flex-1 justify-end bg-[#000]/[.2]'>
                            <View className=' items-center space-y-5 pb-2 pt-5 bg-white w-full rounded-t-[30px] ' style={{ elevation: 2 }}>
